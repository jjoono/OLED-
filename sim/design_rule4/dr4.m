...
if ~exist('NSUB','var'),  NSUB=1.77; end
if ~exist('DETL','var'),  DETL=500;  end
if ~exist('DHTL','var'),  DHTL=500;  end
...
%% JO structure %%
no_bar=[1 NAG+KAG*1i NORG+KORG*1i NORG NORG+KORG*1i NTCO+KITO*1i NSUB];
...
for k1=1:length(d1)
    for k2=1:length(d2)
        thickness=[DAG DETL DEML DHTL DTCO];   % Ag, ETL, EML, HTL, TCO
        switch MODE
          case 'kito', no_bar(6)=NTCO+d1(k1)*1i;
        ...
        extra_matrix(k_all,:)=[d1(k1),eta_sub,Aprime,eta_ext,eta_sub*eta_ext];
...
csvwrite(OUTCSV,extra_matrix); toc;   % columns: param, eta_sub, Aprime, eta_ext(per p), EQE(per p)
