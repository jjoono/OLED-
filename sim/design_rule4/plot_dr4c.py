...
(a) reads dr4c_ito.csv, (b) reads dr4c_ag.csv.
...
ito = np.loadtxt('dr4c_ito.csv', delimiter=',')
ag  = np.loadtxt('dr4c_ag.csv',  delimiter=',')
...
axes[0].set_title('(a)  device A — TCO electrode, 50 nm:  $\\bar n=1.8+k_{TCO}i$', fontsize=11, loc='left')
...
foot = ('Two ALTERNATIVE devices, not one stack. Common part, 550 nm, isotropic dipole, PLQY = 1:  '
        ...
        'Device A takes a 50-nm TCO, device B a 10-nm thin Ag; the 100-nm reflector is never swept.  '
        'All organics n = 1.8 with k = 0; substrate n = 1.8, index-matched, so the waveguide channel is identically zero; five-channel closure 3e-15.  '
...
fig.savefig('dr4c_mock.png', dpi=150)
print('saved')
