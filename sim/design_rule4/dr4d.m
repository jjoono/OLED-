...
diag_matrix = zeros(Nd1*Nd2,9); ne_rows=zeros(Nd1*Nd2,1); extra_matrix=zeros(Nd1*Nd2,6+2*length(PESC));   % v2: [d1, abs_top, abs_bottom, sub_TIR, sub_recycle_abs, sum5, nonrad, residual, Purcell]
for k1=1:length(d1)
    for k2=1:length(d2)
        ...
        extra_matrix(k_all,:)=[d1(k1),eta_sub,Aprime,EQE_wg,EQE_spp,EQE_abs,eta_ext,eta_sub*eta_ext];
