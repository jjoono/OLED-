...
% evanescent branch fix: force imag(n*cos) on one side (sign from TMF_SIGN, default +), see run notes
sg = 1; if strcmp(getenv('TMF_SIGN'), '-'), sg = -1; end
fp = sg*imag(no_bar_u_num_1.*cos_theta_p) < 0; cos_theta_p(fp) = -cos_theta_p(fp);
fs = sg*imag(no_bar_u_num_1.*cos_theta_s) < 0; cos_theta_s(fs) = -cos_theta_s(fs);
