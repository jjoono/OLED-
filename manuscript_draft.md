...
The benchmark of Sections 2.1–2.3 concerns one array construction: convex lenslets molded on the substrate exit face. To check that the observed saturation is not an artifact of this particular construction, we subject three further manufacturable MLA families to the same test: (i) an **embedded MLA**, in which the lens array is planarized under a low-refractive-index resin overcoat ($n \approx 1.41$) so that the outer optical surface is flat, following the embedded-MLA architecture of Qu et al. [5]; (ii) an **inverted (concave) MLA**, the concave counterpart of the same freeform profile class, as realized in inverted-microlens-array platforms [16]; and (iii) a **randomly assembled MLA**, a non-periodic array represented by a pseudo-random supercell in which the lenslet positions are jittered off the hexagonal lattice and each lenslet carries an independently drawn random profile from the same class, the supercell then being tiled periodically (Methods 4.5). The supercell representation is statistically equivalent to a truly random array provided the correlation length of the disorder is well below the supercell size.

Each family receives a lightweight version of the full protocol: 100 random feasible designs to populate the achievable region, one dedicated optimization of total EQE, and high-precision re-evaluation of the best candidates. For each family we check the three signatures of saturation established above: (a) near-linear collapse of the (total EQE, band EQE) points, (b) selectivity of the best design close to the Lambertian partition, and (c) the same weak, monotonic selectivity drift. The results are pending: **[embedded MLA stress test: TO BE INSERTED]**; **[inverted MLA stress test: TO BE INSERTED]**; **[randomly assembled MLA stress test: TO BE INSERTED]**. Figure 5 will carry these results as a three-panel-per-family generality check. If the three signatures recur, the saturation statement extends from a single array construction to the four tested manufacturable families; if a family departs from them, that departure itself localizes which construction feature matters.

**Fig. 5 | Generality check across MLA families.** For each of the three additional families—embedded (low-index overcoat, $n \approx 1.41$, flat top), inverted (concave), and randomly assembled (pseudo-random supercell)—three panels: (left) total EQE versus 40–60° band EQE for the 100 random designs and the dedicated optimum **[TO BE INSERTED]**; (middle) band selectivity of the best design against the Lambertian partition **[TO BE INSERTED]**; (right) selectivity–efficiency correlations $R$ per band **[TO BE INSERTED]**.
