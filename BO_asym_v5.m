% ============================================================
%  BO_asym_v5.m  -- v4 + MLA 패턴 크기(r_pat) 변수화
%
%  [v4 대비 변경]
%   - r_pat(텍스처 zone 크기 = MLA 패턴 크기)을 최적화 변수로 추가. 범위 [2,25].
%   - Decenter 는 절대값 대신 dec_frac(셀 대비 비율, 0~0.5)로 재정의:
%       실제 편심 = dec_frac * r_pat  -> r_pat 이 변해도 항상 셀 안에 유지.
%   - @@CONFIRM(C9): ReferenceSurface 기준 X=15 가 "고정 상수"인지 "r_pat 을
%       따라가야 하는 값"인지 모델 확인 필요. 기본은 고정 15 + 편심. 셀 기준이면
%       objFcn 내 표시된 줄로 교체.
%  ---- 이하 v4 헤더 ----
%
%  [v3 대비 변경]
...
varNames = [shapeNames, {'dETL','dHTL','dAg','stretchZ','dec_frac','r_pat'}];
lb = [lbS,  10,  10,  0,  0.1, 0.0,  2];
ub = [ubS, 150, 150, 50,  3.0, 0.5, 25];
NV = numel(lb);   S_DIM = 4*NRBF + 3*NCUT;
fprintf('총 변수 = %d (형상 %d = RBF %d*4 + 절단 %d*3 | + dETL,dHTL,dAg,stretchZ,dec_frac,r_pat)\n', ...
    NV, S_DIM, NRBF, NCUT);
...
stretchZ = point(S+4);  dec_frac = point(S+5);  r_pat = point(S+6);
Decenter = dec_frac * r_pat;   % 편심 = 셀 대비 비율 -> 항상 셀 안
...
ltml.LTDbSet(lt,Key,'X',15+Decenter);            % @@CONFIRM(C9) 기준 X=15 고정 가정
% ltml.LTDbSet(lt,Key,'X',r_pat+Decenter);       % <- 기준이 셀 크기를 따라가면 이 줄로 교체
...
fprintf('[obj] EQE_region=%.5g | EQE_total=%.5g | φc=%+.0f° | contrast=%.2f | draft=%.0f° | dAg=%.1f r_pat=%.1f dec=%.2f%s\n', ...
    PWin, EQE_total, phiC, contrast, maxDraft, dAg, r_pat, Decenter, ternary(loadFailed,' [LOAD FAIL]',''));
