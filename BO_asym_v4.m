% ============================================================
%  BO_asym_v4.m  -- 고DOF(절단면 포함) 비대칭 freeform + Ag 캐비티 공동최적화
%
%  [v3 대비 변경]
...
varNames = [shapeNames, {'dETL','dHTL','dAg','stretchZ','Decenter'}];
lb = [lbS,  10,  10,  0,  0.1, 0.0];
ub = [ubS, 150, 150, 50,  3.0, 7.5];
NV = numel(lb);   S_DIM = 4*NRBF + 3*NCUT;
fprintf('총 변수 = %d (형상 %d = RBF %d*4 + 절단 %d*3 | + dETL,dHTL,dAg,stretchZ,Decenter)\n', ...
    NV, S_DIM, NRBF, NCUT);
...
optsSurr = optimoptions('surrogateopt', ...
    ...
    'PlotFcn', [], ...
    'CheckpointFile', 'BO_asym_v4_ckpt.mat', ...
    'Display', 'iter');
...
save('BO_asym_v4_result.mat', 'xBest','bestEQE','bestStd','bd','varNames','lb','ub', ...
    'r_pat','NRBF','NCUT','TH_LO','TH_HI','PHI_W','outS','OPT_HIST');
fprintf('\n######## Done ########\n');
...
stretchZ = point(S+4);  Decenter = point(S+5);
...
fprintf('[obj] EQE_region=%.5g | EQE_total=%.5g | φc=%+.0f° | contrast=%.2f | draft=%.0f° | dAg=%.1f dec=%.2f%s\n', ...
    PWin, EQE_total, phiC, contrast, maxDraft, dAg, Decenter, ternary(loadFailed,' [LOAD FAIL]',''));
