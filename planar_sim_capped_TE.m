function out = planar_sim_capped_TE(dHATCN, dCap, eta_rad, hdr, z0_in, dTAPC)
% Planar air-mode dipole simulation for the capped thin-metal top-emission stack
%   Air | Al 100 | Bebq2 50 | EML 25 (Bebq2:Ir(mphmq)2tmd) | TAPC | HAT-CN | Ag 15 | ZnS cap | Air
% (the Batch10 structure). Ag uses the measured nk of a 15 nm film on HAT-CN.
%
% Inputs: dHATCN, dCap, dTAPC in nm; z0_in = dipole position in nm measured from
% the Bebq2(ETL)/EML interface; eta_rad, hdr = radiative efficiency and horizontal
% dipole ratio.
% Requires the fixed TMF_birefringence_whole* (evanescent branch cut).
%
% The capping-layer thickness is the dominant optical variable of this stack:
% EQE_air runs 12.6% (no cap) -> 40.1% (HAT-CN 15 nm, ZnS 65 nm) at z0 = 24 nm.
% Example: out = planar_sim_capped_TE(20, 60, 0.96, 0.82, 24, 65);
...
% Batch10 stack: Air|Al100|Bebq2 50|EML 25|TAPC|HATCN|Ag15|ZnS cap|Air
no_bar=[ones(wavelength_num,1) material.l_Al real(material.l_bebq2_JH) real(material.l_bebq2_JH) material.l_TAPC_o_JO material.l_HATCN material.l_Ag_15_onHATCN material.l_ZnS ones(401,1)];
ne_bar=[ones(wavelength_num,1) material.l_Al real(material.l_bebq2_JH) real(material.l_bebq2_JH) material.l_TAPC_e_JO material.l_HATCN material.l_Ag_15_onHATCN material.l_ZnS ones(401,1)];

EML_position=4;
thickness=[100 50 25 dTAPC dHATCN 15 dCap];   % [Al Bebq2 EML TAPC HATCN Ag ZnS]
z0=z0_in;
u_data_num=998;
max_u=3;
