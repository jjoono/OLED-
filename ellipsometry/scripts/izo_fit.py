"""IZO fits: analytic Drude + UV interband B-spline(e2, KK) + eps_inf.
import os, pathlib
...
Validates on N2_IZO1 (target: CE d=189.3nm), then applies to O2_IZO3."""
...
W=1.0/(1.0+(np.abs(dep)/3.0)**2)          # depol-based weights

# KK grid/ops (UV interband part only; Drude analytic)
...
# UV e2 nodes (interband, Eg~3.1-3.6): dense near onset, extend to 8eV via tail param
En=np.array([2.6,2.9,3.1,3.3,3.5,3.7,3.9,4.1,4.3,4.6,4.9,5.2,5.6,6.2,7.0,8.0])
NN=len(En)

def eps_layer(p):
    C0=p[0]; Ad=p[1]; gd=p[2]; nodes=np.maximum(p[5:5+NN],0.0)
    e2uv=PchipInterpolator(np.concatenate(([Eg[0],2.2],En)),
                           np.concatenate(([0,0],nodes)))(Eg)
    e2uv=np.maximum(e2uv,0.0)
    e1=C0+kk(e2uv)
    e2=e2uv.copy()
    # analytic Drude
...
def resid(p):
    ...
    return np.concatenate([ (W*(P-psi_m)).ravel(), (W*dd/3.0).ravel() ])

# p = [C0, A_drude, gamma_drude, d, d_rough, nodes...]
p0=np.array([3.0, 1.5, 0.15, d_seed, 1.5]+[0.3,0.5,0.8,1.0,1.2,1.4,1.6,1.8,2.0,2.2,2.4,2.5,2.6,2.7,2.8,2.8])
lo=np.array([1.5, 0.0, 0.02, 150.0, 0.0]+[0]*NN)
hi=np.array([5.0, 30.0, 3.0, 230.0, 12.0]+[20]*NN)
r=least_squares(resid,p0,bounds=(lo,hi),xtol=1e-11,ftol=1e-11,max_nfev=400)
r=least_squares(resid,r.x,bounds=(lo,hi),xtol=1e-12,ftol=1e-12,max_nfev=400)
p=r.x
P,L=forward(p)
dd=(L-del_m+180)%360-180
print('%s fit: d=%.2fnm rough=%.2fnm C0=%.3f  Drude A=%.3f gamma=%.3f'%(which,p[3],p[4],p[0],p[1],p[2]))
print('RMSE psi=%.3f del=%.3f  (weighted resid norm %.1f)'%(
      np.sqrt(np.mean((P-psi_m)**2)),np.sqrt(np.mean(dd**2)),np.sqrt(np.mean(resid(p)**2))))
Nf=eps_layer(p)
