...
OUTDIR   = _os.path.join(ELLIPS_OUT, r'mod_v12')
RESULT   = _os.path.join(ELLIPS_OUT, r'ce_final2_result.json')
...
def blocks(p, free):
    ...
    osc = [("'Drude(RT)'", False, [
           ...
           ("'Tauc-Lorentz'", True, [
              ...
              ('Br',  num(p[5]), free[5], '0.05',   '9.00',   '1.0E-4', '1000.0'),
    ...
    if ng:
        osc.append(("'Gaussian'", False, [
              ...
              ('Br',   num(p[9]),  free[9],  '0.05',    '9.00',   '0.0',     '100.0'),
              ('En',   num(p[10]), free[10], '0.20',    '6.00',   '1.0E-8',  '15.0')]))
...
def main():
    ...
    print('%-16s %4s %6s %7s %7s %7s  %s'%('sample','osc','MSE','d(nm)','rough','dth','locked (no info in 340-1080 nm)'))
    for sh, rec in R.items():
        p = rec['p']; rel = rec['rel']
        free = [rel[j] <= 50.0 for j in range(len(p))]
