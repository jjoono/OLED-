...
OUTDIR=_os.path.join(ELLIPS_OUT, r'mod_files_v3')
...
def eps_of(p,E):
    e2=np.maximum(tl_e2(p[2],p[3],p[4],p[5])+tl_e2(p[6],p[7],p[8],p[9]),0.0)
...
LO=np.array([1.0,0.0,  5.0,3.0,0.05,2.50,  0.0,0.9,0.10,0.30])
HI=np.array([6.0,4.0,400.0,8.0,8.00,4.50, 60.0,3.2,4.00,2.60])
    ...
    for Ad0 in [0.05,0.3,0.9,1.8]:
        for Eg0 in [3.0,3.4,3.8]:
            for C0 in [0.5,1.5,3.0]:
                p0=np.clip(np.array([3.0,Ad0,100.,4.8,C0,Eg0, 1.0,1.9,0.8,1.0]),LO,HI)
...
def main():
    ...
    # --- expand Gen-Osc Grade Parms: one 4-line group per oscillator parameter.
    # Template has 6 groups (Drude 2 + TL 4).  A 2nd Tauc-Lorentz needs 4 more,
    # so append copies of the TL groups (3..6).  Without this CompleteEASE
    # refuses the file outright ("no model loaded").
    gm=re.search(r"(start_Gen-Osc Grade Parms\r\n)(.*?)(\r\n\t\tend_Gen-Osc Grade Parms)",tpl,re.S)
    gl=gm.group(2).split('\r\n')
    assert len(gl)==25, len(gl)
    gl_new=gl[0:24]+gl[8:24]+[gl[24]]
    tpl=tpl[:gm.start()]+gm.group(1)+'\r\n'.join(gl_new)+gm.group(3)+tpl[gm.end():]
    print('grade groups: 6 -> %d'%((len(gl_new)-1)//4))
    ...
    for s in '12345':
        ...
        L=list(tlines)
        L=L[:9]+[tlines[4],tlines[5],tlines[6],tlines[7],tlines[8]]+L[9:]
        L[0]=setf(L[0],0,'3')
        ...
        # line10..13: 2nd Tauc-Lorentz (sub-gap band)  Amp, Br(=C2), Eo(=E02), Eg
        for idx,(v,lo,hi) in zip((10,11,12,13),
                                 ((p[6],'0.0','60.0'),(p[8],'0.05','8.0'),
                                  (p[7],'0.5','4.0'), (p[9],'0.2','3.0'))):
            L[idx]=setf(L[idx],0,repr(float(v))); L[idx]=setf(L[idx],2,lo); L[idx]=setf(L[idx],3,hi)
        newblock='\r\n'.join(L)
        ...
        print('%-16s Einf=%.4f rho=%.4g tau=%.3f | TL1(A=%.1f Br=%.3f Eo=%.3f Eg=%.3f) | dn=%.4f dk=%.4f | lines=%d'%(
    ...
    with open(os.path.join(OUTDIR,'ITO_IZO_nk_v3.csv'),'w',newline='') as f:
        wtr=csv.writer(f)
