% ============================================================
%  BO_asym_v6_DBR.m  -- v6(DBR) 에서 Ag 전극 제거 + ITO 150nm 단독 버전
%
%  [이번 변경 (Ag 제거 -> ITO 150nm)]
%   - 하부 전극에서 Ag 를 아예 제거(두께 0)하고, ITO(150nm) 한 층만 사용.
%   - 그 아래 DBR 은 그대로 유지. 즉 스택은
...
%   - 본 파일: 그 v6 에서 다시 Ag 를 빼고 ITO 150nm + DBR 만 남긴 케이스.
%  ---- 이하 v4 헤더 ----
%
%  [v3 대비 변경]
...
%   (C8) 소자: TAPC 다음층이 ITO(150nm) 고정, 그 아래 DBR. nk_JH_total.mat 의
%        material 구조체에 ITO 필드(l_ITO_SNU_temp)가 있어야 함(사용자 확인).
% ============================================================
clear;
...
S = 4*NRBF + 3*NCUT;
pShape   = point(1:S);
dETL     = point(S+1);  dHTL = point(S+2);        % <-- dAg 제거로 인덱스 재정렬
stretchZ = point(S+3);
...
wavelength_start=595; wavelength_end=605; n=10;
...
ltml.LTDbSet(lt,Key,'Y',15+d_sub+0.105);   % @@CONFIRM 모델의 ReferenceSurface Y 기본값이 15가 아니면 그 값으로 교체
...
ito_col = material.l_ITO_SNU_temp;   ito_th = tITO;

no_bar = [ones(401,1) material.l_Al_JO material.l_B3_o_JO material.l_TCTA_B3_o_JO ...
          material.l_TCTA_o_JO material.l_TAPC_o_JO  ito_col dbr_cols  1.51*ones(401,1)];
ne_bar = [ones(401,1) material.l_Al_JO material.l_B3_e_JO material.l_TCTA_B3_e_JO ...
          material.l_TCTA_e_JO material.l_TAPC_e_JO  ito_col dbr_cols  1.51*ones(401,1)];
thickness = [100 dETL 25 10 dHTL  ito_th dbr_th];   % <-- Ag 제거, ITO 150nm + DBR
