...
% <<< wavelength MUST be a column vector >>>  every per-wavelength quantity
% below is a column, so a row here silently turns const into a
% wavelength_num x wavelength_num matrix and makes TMF_birefringence_whole fail.
wavelength=(530:580)';
wavelength_num=length(wavelength);

emission_spectrum=spectrum.I_Irppy2acac(131:181);
% normalise: every weight downstream assumes sum(emission_spectrum) = 1
...
%% JO ���� %%
W=131:181;   % 530-580 nm on the 400-800 nm grid of the material library
no_bar=[ones(wavelength_num,1) material.l_Ag_McPeak(W) material.l_B3_o_JO(W) material.l_TCTA_B3_o_JO(W) material.l_TAPC_o_JO(W) material.l_ITO(W) 1.5*ones(wavelength_num,1)];
ne_bar=[ones(wavelength_num,1) material.l_Ag_McPeak(W) material.l_B3_e_JO(W) material.l_TCTA_B3_e_JO(W) material.l_TAPC_e_JO(W) material.l_ITO(W) 1.5*ones(wavelength_num,1)];
...
for k1=1:length(d1)
    for k2=1:length(d2)
        ...
        % photon-number weighting: EQE counts photons, so each wavelength carries
        % a factor lambda (photon energy ~ 1/lambda). Identical to the old
        % expression at a single wavelength, different as soon as there are several.
        photon_weight = wavelength.*emission_spectrum.*eta_eff./sumUtot ...
                        ./ sum(wavelength.*emission_spectrum);
