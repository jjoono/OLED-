...
**발광체는 렌즈 캡 하나뿐입니다.** 예전에는 상면에 발광 평면을 따로 깔았는데,
패널 옆면 실루엣에서 그 평면이 거의 옆에서 보이면서 가장자리에 밝은 초록 띠가
생기고, 캡들이 그 띠를 톱니처럼 잘라 먹는 인공물이 나왔습니다. 평면을 없애고
`LENS_EMIT_FRAC = 1.0` 으로 캡이 전부를 들고 가면 인공물이 사라지고, 덤으로
"빛은 광추출 구조를 통해 나온다"는 그림의 논지와도 맞습니다.

발광체·렌즈는 `visible_diffuse = visible_glossy = False` 입니다. 켜 두면 한쪽
소자의 글로우가 옆 소자의 가까운 모서리를 물들입니다.
