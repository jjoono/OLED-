...
- **발광 프로파일 = 구동된 둥근 픽셀 + 그 바깥의 확산 헤일로**입니다.
  ...
  발광 평면과 렌즈 캡이 같은 `radial_falloff` 노드를 공유하므로 어긋나지 않습니다.
...
| 요소 | 오브젝트 / 함수 | 역할 |
|---|---|---|
| 7층 스택 | `LAYERS` | Al / ETL / EML / HTL / ITO / Glass / MLA film. 층마다 고유색 |
| 층 라벨 | `stack_label()` | 앞면에 3D 텍스트. Al 층만 밝은 잉크(대비) |
| 엑시톤 | `excitons()` | EML 안, 구동된 픽셀 바로 아래 |
| **도파 광선** | `guided_rays()` | 픽셀에서 옆으로 새어나가 외부면과 음극 사이를 **튕기며 퍼지는 경로** |
| 마이크로렌즈 | `lens_array()` | 육각 배열 구면 캡 |
| 발광 픽셀 · 빛기둥 | `emitter_material()`, `haze_material()` | |

층은 굴절(transmission)이 아니라 **알파 블렌딩**입니다. Cycles에서 진짜 투과는
뒤에 있는 것을 오히려 가리고 비용도 큽니다.

도파 광선은 슬래브 내부가 아니라 **앞면 평면**에 그립니다. 내부에 두면 그 위의
필름과 렌즈 어레이에 가려서 유리를 아무리 투명하게 해도 보이지 않습니다.
라벨이 같은 면에 있으므로 단면도로 읽힙니다. 광선은 한쪽(오른쪽)으로만 나갑니다 —
왼쪽 절반은 라벨 자리이고, 원형 확산은 상면 글로우가 이미 보여줍니다.

광선의 **길이는 패널 끝이 아니라 세기로 잘립니다**(`PIX_R + lam*2.8`). 둘 다
가장자리에서 자르면 도달 거리 차이가 사라져서, 비교 자체가 안 보입니다.
