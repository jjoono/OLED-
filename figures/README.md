...
3D 렌더는 **뒷면 거울이 두 소자의 차이**입니다 — 왼쪽은 `Al electrode`(R 0.75),
오른쪽은 `Ag electrode`(R 0.98). 라벨이 주장하는 바를 손실 모델의 `r_met` 이
그대로 받습니다. 두 패널의 차이는 **1.99× / 1.67×** 입니다. 2D 그림(Fig.1a)의 2.32× / 1.84× 는 메커니즘을 또렷이
...
- **세번째 뷰: 아이소메트릭 + 층 안 라벨** (`inside_figure()`, `*_boxed.pptx`).
  글자를 층 안에 넣으려면 투영을 **눕혀야** 합니다. 폭 W 인 가로 라벨이 띠 안에
  들어가려면 띠 두께가 `H + (기울기)*W` 이상이어야 하는데, 보통의 30° 아이소메트릭은
  기울기가 0.58 이라 140 폭 라벨에 100 두께가 필요합니다. `IEX/IEY = 0.94/0.26`
  (기울기 0.28)으로 눕히면 55 면 됩니다. 층 두께는 `IMIN`/`IMIN2` 로 보장합니다.
- 라벨은 **오른쪽 옆면**에 얹습니다(참고한 도식들이 그렇게 합니다). 그래서 라벨이
  쓸 수 있는 폭은 `IEX*IBD` 이고, 블록은 `IBW`/`IBD` = 300/215 로 두툼하게 둡니다.
- 정면뷰는 뒷변이 `FDY` 만큼 올라가고 `FSPLAY` 만큼 **중심선 기준으로 벌어집니다.**
  층 하나가 좌우 대칭인 사다리꼴 쐐기가 되고, **양 옆면이 정면 실루엣 바깥으로
  나와서 똑같이 보입니다.** 한쪽으로 미는(shear) 방식은 반대쪽 면이 아예 안 보이고,
  뒤를 좁히면 양쪽 다 정면 뒤로 숨습니다 — 정중앙에서 본 상자의 옆면은 원래 딱
  모서리로만 보이니까요. 좌우 면 밝기는 같게 둬서 한쪽만 조명 받은 것처럼 안 보이게
  했습니다.
...
- **두 소자를 따로 렌더합니다** (`emitting_area_render_conventional.png`,
  `_designrule.png`, 각 1400×1400). 합쳐서 렌더하면 둘 사이 간격이 이미지에 박혀서
  배치할 때 다시 렌더하지 않고는 못 고칩니다.
- **그림자가 없는 완전 투명 배경입니다.** 바닥면을 지우는 대신
  `visible_camera = False` 로 둡니다. 지워버리면 바운스 광도 같이 사라져서, 금속
  층이 반사할 게 어두운 월드 그라디언트밖에 없어 **거의 검게** 나옵니다. 카메라에만
  안 보이게 하면 평면도 접지 그림자도 안 나오면서 조명은 그대로입니다.
