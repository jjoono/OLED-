...
%% JO structure %%
no_bar=[ones(wavelength_num,1) material.l_Ag_McPeak(151) 1.8 2+0.02i 1.5];
ne_bar=no_bar;

d1= 10:20:500;   % d_org
Nd1=length(d1);
d2= [1.5 1.65 1.8];   % n_sub
Nd2=length(d2);
data_matrix = zeros(Nd1*Nd2,12);
disp_matrix = zeros(Nd1*Nd2,6);
diag_matrix = zeros(Nd1*Nd2,9); ns_rows=zeros(Nd1*Nd2,1);   % v2: [d1, abs_top, abs_bottom, sub_TIR, sub_recycle_abs, sum5, nonrad, residual, Purcell]
for k1=1:length(d1)
    for k2=1:length(d2)
        no_bar(:,5)=d2(k2); ne_bar(:,5)=d2(k2);
        thickness= [100 d1(k1) 50];
        ...
        disp_matrix(k_all,:)=[d1(k1),EQE_air,EQE_sub_confined,EQE_wg,EQE_spp,EQE_abs]; ns_rows(k_all)=d2(k2);
...
csvwrite('fig3a_Ag.csv',[ns_rows(:) disp_matrix]); toc;
