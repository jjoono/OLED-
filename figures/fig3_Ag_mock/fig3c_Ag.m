...
% fig3c: 2-D map eta_sub(d_ETL, n_sub) for a fixed extraordinary index of the ETL.
% Run as:  octave --eval "NE_ETL=1.8; OUTCSV='fig3c_Ag_ne18.csv'; fig3c_Ag"
if ~exist('NE_ETL','var'),  NE_ETL=1.8; end
if ~exist('OUTCSV','var'),  OUTCSV='fig3c_Ag.csv'; end
wavelength=550;
...
%% JO structure %%
no_bar=[ones(wavelength_num,1) material.l_Ag_McPeak(151) 1.8 1.8 1.8 1.8+0.02i 1.8];
ne_bar=no_bar;

d1= [5 10 15 20 30 40 50 60 70 80 100 120 140 160 180 200 225 250 300 350 400];   % d_ETL (nm)
Nd1=length(d1);
d2= 1.40:0.05:2.00;  % n_sub (substrate / MLA index)
Nd2=length(d2);
data_matrix = zeros(Nd1*Nd2,12);
disp_matrix = zeros(Nd1*Nd2,6);
diag_matrix = zeros(Nd1*Nd2,9); ne_rows=zeros(Nd1*Nd2,1);   % v2: [d1, abs_top, abs_bottom, sub_TIR, sub_recycle_abs, sum5, nonrad, residual, Purcell]
for k1=1:length(d1)
    for k2=1:length(d2)
        ne_bar(:,3)=NE_ETL;              % ETL extraordinary index (fixed per map)
        no_bar(:,7)=d2(k2); ne_bar(:,7)=d2(k2);   % substrate index (map y-axis)
        thickness= [100 d1(k1) 20 50 50];   % Al, ETL, EML, HTL, ITO

        EML_position=4;

        %         z0=d_slice/2; % emitting position in EML
        z0=10;   % dipole at the centre of the 20-nm EML
        u_data_num=997;
        max_u=3;
...
csvwrite(OUTCSV,[ne_rows(:) disp_matrix]); toc;   % columns: n_sub, d_ETL, air, sub_confined, wg, spp, abs
