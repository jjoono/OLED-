...
rows = [
 ('Panels to redraw', 'every eta_ext / EQE below is the matrix series of eq. (1) with the hemispherical MLA BSDF (n_MLA = n_sub); p is the cos.sin-weighted B_T of the structure'),
 (None, None),
 ('Fig. 2(c)', 'eta_ext vs ITO thickness, n_sub 1.5, Al and Ag.  Was eq. (3) with p = 0.38.  Not monotonic any more (Ag 0.91 / 0.95 / 0.94 / 0.91 at 50 / 100 / 150 / 200 nm); Ag above Al by 12-15 %p everywhere.'),
 ...
 ('Fig. 2(e)', 'eta_ext vs n_sub, series.  Gap Ag - Al: 7 %p (1.3), 13 %p (1.5), 21 %p (2.0).'),
 ('Fig. 2(f)', 'eta_sub (unchanged) and EQE vs n_sub, series; extraction loss = eta_sub - EQE.  Al plateaus at 55-59 % for n >= 1.8 (was a 63 % maximum); Ag 84 % at 1.8, 87 % at 2.0 (was 89 %).'),
...
sheet('Fig2c', ['d_ITO_nm', 'Aprime_Al', 'eta_ext_Al', 'Aprime_Ag', 'eta_ext_Ag'],
      [[float(a['d_ITO_nm']), float(a['Aprime']), float(a['eta_ext_series']), float(g['Aprime']), float(g['eta_ext_series'])] for a, g in zip(al, ag)])
...
sheet('Fig2e', ['n_sub', 'eta_ext_Al', 'eta_ext_Ag'], [[float(a['n_sub']), float(a['eta_ext_series']), float(g['eta_ext_series'])] for a, g in zip(al, ag)])
sheet('Fig2f', ['n_sub', 'eta_sub_Al', 'EQE_Al', 'extraction_loss_Al', 'eta_sub_Ag', 'EQE_Ag', 'extraction_loss_Ag'],
      [[float(a['n_sub']), float(a['eta_sub']), float(a['EQE_series']), float(a['eta_sub'] - a['EQE_series']), float(g['eta_sub']), float(g['EQE_series']), float(g['eta_sub'] - g['EQE_series'])] for a, g in zip(al, ag)])
