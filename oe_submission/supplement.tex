...
\section*{Table S1 | Common platform}
...
Textured patch & 25 \ensuremath{\times} 25 mm (all campaigns) \\
...
0–20\ensuremath{^\circ}, 20–40\ensuremath{^\circ}, 40–60\ensuremath{^\circ}, 60–80\ensuremath{^\circ}. Band selectivity is
...
\section*{Table S2 | Per-campaign settings}
...
\hline
Convex, weighted sweep & \texttt{pareto\_front\_freeform.m} & 13 & 150 random + 120/weight + 15 polish, $w \in \{0,\,0.25,\,0.5,\,0.75,\,1\}$ & 10,000 rays, 31 λ & 50,000 rays, 151 λ, \ensuremath{\times}3 & phase 1–3 \\
Convex, per-band & \texttt{opt\_4band\_freeform.m} & 13 & 60 + 15 polish per arm, 5 arms & 10,000 rays, 31 λ & 50,000 rays, 151 λ, \ensuremath{\times}3 & phase 4 \\
Convex, total EQE only & \texttt{freeform\_EQEtotal.m} & 13 & 140 per start, 3 independent starts & 10,000 rays, 31 λ & 50,000 rays, 151 λ, \ensuremath{\times}3 & — \\
Hemisphere-seeded control & \texttt{warmstart\_from\_hemisphere.m} & 13 & 40 + 15 + 15 polish per arm, 4 arms & 10,000 rays, 31 λ & 50,000 rays, 151 λ, \ensuremath{\times}3 & phase 9 \\
Hemispherical reference & \texttt{opt\_hemisphere\_arms.m} & 3 (cavity + height; shape fixed on a quarter circle) & 30 + 10 polish per arm, 5 arms & 10,000 rays, 31 λ & 50,000 rays, 151 λ, \ensuremath{\times}3 & phase 8 \\
Inverted (concave) & \texttt{opt\_4band\_inverted.m} & 13 & 60 + 15 polish per arm, 4 arms (identical protocol to the convex benchmark) & 10,000 rays, 31 λ & 50,000 rays, 151 λ, \ensuremath{\times}3 & phase 6 \\
Randomly assembled & \texttt{stress\_random\_mla.m} & 6 assembly statistics & 50 random + 60 + 15 polish & 5,000 rays, 16 λ & 10,000 rays, 151 λ, \ensuremath{\times}3 seeds & phase 7 \\
Convergence check & \texttt{convergence\_check.m} & — (re-evaluation only) & 20 designs re-evaluated & — & 200,000 rays \ensuremath{\times}3, and 450–750 nm broadband & — \\
Patch-size check & \texttt{check\_patch\_convergence.m} & — (re-evaluation only) & 1 design \ensuremath{\times} 3 patch sizes \ensuremath{\times} 3 repeats & — & 50,000 rays, 151 λ & phase 11 \\
20–40\ensuremath{^\circ} confirmation & \texttt{reeval\_confirm\_2040.m} & — (re-evaluation only) & 2 designs \ensuremath{\times} 5 repeats & — & 50,000 rays, 151 λ & phase 10 \\
\hline
\end{tabular}
\end{table}

The randomly assembled family is the only campaign with reduced sampling. Its
supercell carries 6 \ensuremath{\times} 6 lenslets on a 141 \ensuremath{\times} 141 height grid, so one evaluation
...
(\texttt{LibraryElementUnitCell.Bumps}, "Yes" \ensuremath{\rightarrow} "No"), so the two differ in
construction and in nothing else.

\section*{Table S3 | Cost calibration for the randomly assembled family (Fig. S3)}

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/figS3_cost_calibration.pdf}
\caption{Fig. S3. Graphical form of this section's data.}
\end{figure}
...
coarser λ & 8 \ensuremath{\times} 8 & 201 & 10,000 & 20 nm & 16 & 446.5 & 1.74 & 0.5012 & 0.66 \\
...
The adopted combination is 4.3\ensuremath{\times} faster while shifting selectivity by 0.39
...
\section*{Table S4 | Convergence of the selectivity–efficiency drift (Fig. S4)}

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/figS4_convergence.pdf}
\caption{Fig. S4. Graphical form of this section's data.}
\end{figure}
...
\section*{Table S5 | Family comparison}

Two statistics are given for each family, both computed from that family's own
evaluation log: the \textbf{top-20 median}, i.e. the median selectivity of the twenty
designs with the highest total EQE, and the \textbf{population mean} over every
...
\ensuremath{\dagger} Large-patch normalization: this campaign's absolute totals correspond to the
...
Every entry lies within 0.094–0.113 (0–20\ensuremath{^\circ}), 0.274–0.303 (20–40\ensuremath{^\circ}),
0.340–0.361 (40–60\ensuremath{^\circ}) and 0.213–0.244 (60–80\ensuremath{^\circ}). For the randomly assembled
...
\hline
0–20\ensuremath{^\circ} & 0.06699 & 0.06682 & 1.003 \\
20–40\ensuremath{^\circ} & 0.16628 & 0.16591 & 1.002 \\
40–60\ensuremath{^\circ} & 0.19802 & 0.18503 & 1.070 \\
60–80\ensuremath{^\circ} & 0.14075 & 0.13589 & 1.036 \\
total EQE & 0.5539 & 0.54679 & 1.013 \\
\hline
\end{tabular}
\end{table}

Every entry in this table is from a fixed-patch (25 \ensuremath{\times} 25 mm) campaign, so the
two columns share one normalization. The 0–20\ensuremath{^\circ}, 20–40\ensuremath{^\circ} and 60–80\ensuremath{^\circ} freeform entries
come from the hemisphere-seeded control (Table S6) — in the 60–80\ensuremath{^\circ} arm the
control surpassed the per-band campaign's own optimum (0.14075 against
0.13863); the 40–60\ensuremath{^\circ} entry is from the per-band campaign, which the control did
...
\section*{Table S6 | Hemisphere-seeded control (Fig. S2)}

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/figS2_warmstart_control.pdf}
\caption{Fig. S2. Graphical form of this section's data.}
\end{figure}
...
\ensuremath{\ddagger} The 20–40\ensuremath{^\circ} arm was the one borderline case, and was settled by a dedicated
confirmation run: both stored designs re-measured with five fresh repeats each,
search excluded. This gives hemisphere 0.16583 \ensuremath{\pm} 0.00015 against warm start
0.16631 \ensuremath{\pm} 0.00021, a residue of +0.29\% at $t = 4.1$ (five-repeat threshold
1.86) — statistically real, three tenths of a percent in size
(\texttt{reeval\_confirm\_2040\_result.mat}).
...
essentially absent margin rather than a weak search. In the 60–80\ensuremath{^\circ} arm the
...
rejected on geometry; the 60–80\ensuremath{^\circ} arm and the confirmation run completed in a
second overnight session.

\section*{Table S7 | Patch-size dependence (Fig. S1)}

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/figS1_patch_dependence.pdf}
\caption{Fig. S1. Graphical form of this section's data.}
\end{figure}
...
(\texttt{patch\_convergence\_result.mat}); the 100 mm row is 2 repeats from a follow-up
run (\texttt{patch\_convergence\_100.mat}).
...
\item The initial rise saturates with a decay length of about 9 mm — four critical-angle round trips of 2.32 mm each — but the far tail does not: between 35 and 100 mm the total still gains 2.2\%, about three times what a single-exponential tail fitted to the first three points would allow. (An earlier draft of this table extrapolated an infinite-patch limit of 0.5554 from the first three points alone; the 100 mm measurement falsifies that extrapolation, and it is withdrawn.) Multiply recycled light migrates laterally over many tens of round trips before escaping, so the total EQE of a finite film increases slowly with its extent, and the 25 mm value is a lower bound sitting 3.7\% below the 100 mm value.

\item The angular composition is patch-converged: while the total gains 3.8\% from 25 to 100 mm, no band selectivity changes by more than 0.24 pp over the same fourfold enlargement (0.11 pp between 25 and 35 mm). All ratios, correlations and class comparisons, which are evaluated at equal patch, are unaffected by the patch choice.

\item The weighted-sweep campaign's archived 0.5556 falls between the 35 mm and 100 mm values of the same design (0.5513 and 0.5636), while the design re-measures at 0.5428 on the fixed 25 mm patch. That campaign's absolute normalization therefore corresponds to a larger effective patch than the unified 25 mm one, which is why its totals are excluded from the fixed-patch $G_j$ comparison of Table S5.

\end{enumerate}

\section*{Reproducing the figures}

All five manuscript figures are produced by \texttt{make\_figures.py}; the
supplementary figures S1–S4 and the raw-data workbooks by
\texttt{make\_supp\_figures\_and\_data.py}, run from the repository root in that order:
...
quoted in the captions to \texttt{figure\_numbers.txt}, and writes each figure as both
PNG (300 dpi) and PDF. The second exports, for \textbf{every} figure (main and
supplementary), an Excel workbook named after the figure — e.g.
\texttt{fig2\_achievable\_region.xlsx} alongside \texttt{fig2\_achievable\_region.png} — with one
sheet per panel containing exactly the plotted arrays, so any figure can be
re-plotted or restyled from its workbook without touching the \texttt{.mat} archives.
...
\hline
Fig. 1 & \texttt{fig1\_platform.png} / \texttt{.pdf} & \texttt{opt\_4band\_result\_25by25.mat}, \texttt{opt\_hemisphere\_result.mat} \\
Fig. 2 & \texttt{fig2\_achievable\_region.png} / \texttt{.pdf} & \texttt{pareto\_front\_result.mat}, \texttt{opt\_4band\_result\_25by25.mat}, \texttt{opt\_hemisphere\_result.mat}, \texttt{warmstart\_hemisphere\_result.mat}, \texttt{freeform\_EQEtotal\_result.mat} \\
Fig. 3 & \texttt{fig3\_selectivity\_map.png} / \texttt{.pdf} & \texttt{pareto\_front\_result.mat} \\
Fig. 4 & \texttt{fig4\_recycling\_routes.png} / \texttt{.pdf} & \texttt{angular\_recycling\_result.npz}, \texttt{angular\_recycling\_bandwidth.npz} \\
Fig. 5 & \texttt{fig5\_families.png} / \texttt{.pdf} & \texttt{opt\_4band\_result\_25by25.mat}, \texttt{opt\_4band\_inverted\_result.mat}, \texttt{stress\_random\_result.mat} \\
Fig. S1 & \texttt{figS1\_patch\_dependence.png} / \texttt{.pdf} & \texttt{patch\_convergence\_result.mat}, \texttt{patch\_convergence\_100.mat} \\
Fig. S2 & \texttt{figS2\_warmstart\_control.png} / \texttt{.pdf} & \texttt{warmstart\_hemisphere\_result.mat}, \texttt{reeval\_confirm\_2040\_result.mat} \\
Fig. S3 & \texttt{figS3\_cost\_calibration.png} / \texttt{.pdf} & \texttt{calibrate\_random\_cost.mat} \\
Fig. S4 & \texttt{figS4\_convergence.png} / \texttt{.pdf} & \texttt{convergence\_check\_result.mat} \\
Table S3 & — & \texttt{calibrate\_random\_cost.m} \\
Table S4 & — & \texttt{convergence\_check.m}, \texttt{pareto\_front\_result.mat} \\
\hline
\end{tabular}
\end{table}

The MATLAB plotting scripts \texttt{plot\_fig2a\_selectivity.m} and
\texttt{plot\_fig5\_families.m} remain in the repository as the in-session views used
during the campaigns; the published figures come from \texttt{make\_figures.py}.

Result archives: \texttt{pareto\_front\_result.mat}, \texttt{opt\_4band\_result\_25by25.mat},
\texttt{freeform\_EQEtotal\_result.mat}, \texttt{opt\_hemisphere\_result.mat},
\texttt{opt\_4band\_inverted\_result.mat},
\texttt{stress\_random\_result.mat}, \texttt{calibrate\_random\_cost.mat},
\texttt{warmstart\_hemisphere\_result.mat}, \texttt{reeval\_confirm\_2040\_result.mat},
\texttt{patch\_convergence\_result.mat}, \texttt{patch\_convergence\_100.mat},
\texttt{convergence\_check\_result.mat},
\texttt{angular\_recycling\_result.npz},
\texttt{angular\_recycling\_bandwidth.npz}.
