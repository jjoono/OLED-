...
for k, fn in enumerate(files):
    ...
    with np.errstate(all="ignore"):
        ...
        pix.append((fn, noiseL, offL, pdnoise, LperPD, Ep, n_1_10, n_10_100))
    if (k+1) % 3000 == 0:
        print(f"{k+1}", flush=True)

pd.DataFrame(pts, columns=["file","L","Erel","noiseL","J","V"]).to_csv(os.path.join(d,"points3.csv"), index=False)
pd.DataFrame(pix, columns=["file","noiseL","offL","pdnoise","LperPD","Eplateau","n_1_10","n_10_100"]).to_csv(os.path.join(d,"pixels3.csv"), index=False)
