...
**드리프트 원인 확정**: 실리콘 암전류는 약 8 도마다 2배로 변한다.
챔버 온도가 0.1 도만 움직여도 8.2 mV x 0.9% = 74 uV 로, 관측된 드리프트 크기와 일치한다.
앰프가 아니라 **PD 암전류의 온도 의존성**이 드리프트의 정체다.
...
| 8 | **PD 제로바이어스(photovoltaic) 모드** | 3분 | **가장 유망하다.** 역바이어스를 끄면 암전류가 10~100배 줄고 그 1/f 노이즈(현재 바닥 13 uV)도 같이 준다. 대가인 응답 속도 저하는 DC 측정이라 무관하다. 설정 변경뿐이라 비용 0 |
...
**8번에서 개선이 크면 TIA 발주는 보류할 것.** 순서를 그렇게 잡은 이유다.
