...
## 4-b단계 — 수명이 짧거나 처음 다루는 소자라면 (권장)

저휘도 포인트는 통전 시간이 길어도(40초, duty 50% 로 실제 20초) 열화 관점에서는 안전하다.
1 cd/m2 에서 통과 전하는 면적당 40~400 uC/cm2 로, OLED 의 LT50 에 해당하는 전하량
(1~100 C/cm2)보다 10^4~10^6 배 아래다. 1000 cd/m2 에서 LT50 이 1시간인 극단적으로 약한 소자도
1 cd/m2 로 환산하면 10^5 시간이다.

**진짜 위험은 두 가지다.**

1. **측정 순서** — 일반 JVL 스윕(5~6 V)을 먼저 돌리면 그쪽에서 소자가 상한다.
   **저휘도 스캔을 먼저 할 것.**
2. **느린 과도 응답** — 열화가 아니라 정확도 문제다. 저전류 영역의 OLED 는 전하 축적/트랩
   채움 때문에 초~분 단위로 값이 변할 수 있다. 그러면 40초 평균은 정상상태 값이 아니다.

두 경우 모두 **측정하면서 검출**된다:

- **포인트별 추세** (자동, 항상): 사이클 순서로 직선을 맞춰 처음-끝 변화를 본다.
  큰 변화(목표 +-2% 기준으로 20% 이상)만 경고한다. 기울기는 평균보다 sqrt(12) 배 못 정해지기
  때문에 그 이하는 통계적으로 구분되지 않는다. 정상 소자 20회 시뮬레이션에서 오경보 0회.
- **bracket** (`--bracket`, 권장): 스캔이 끝난 뒤 **첫 전압을 한 번 더 측정**해서 처음 값과
  비교한다. 독립적인 두 측정의 비교라 8% 정도부터 잡힌다. **이쪽이 실질적인 대조 실험이다.**

```powershell
python run_lowlight_scan.py --pixel 1 --voltages 2.6,2.8,3.0,3.3,3.6,4.0 --settle 0.005 --bracket
```

처음과 끝이 측정 오차 안에서 일치하면 **스캔 동안 소자가 변하지 않았다는 직접적인 증거**다.
유의하게 다르면 그 스캔의 저휘도 값은 버리고, `--target 0.05` 로 완화해 총 시간을 줄이거나
소자를 바꿔서 다시 잰다. 재측정 포인트는 `repeat` 컬럼이 True 이므로 평가에서 제외한다.
