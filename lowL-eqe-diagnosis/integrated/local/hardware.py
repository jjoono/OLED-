...
class KeithleySource:
    ...
    def set_low_light_mode(self, on):
        ...
        기본값은 NPLC 1 + autozero OFF 다. 1 cd/m2 를 구동하는 전류는 수십 nA
        수준이라 이 설정으로는 EQE 의 분모(전류)가 오차를 지배한다. 저휘도
        모드에서만 autozero ON + NPLC 10 으로 올리고, 끝나면 반드시 원복한다.

        NPLC 10 + autozero ON 이면 전류 판독 1회가 최대 ~0.6 s 걸려서 기본
        VISA 타임아웃으로는 MEASure:CURRent:DC? 가 VI_ERROR_TMO 로 죽는다.
        모드가 켜져 있는 동안만 타임아웃을 넉넉히 잡고, 끌 때 원복한다.
        """
        self.mutex.lock()
        if on:
...
class KeithleyMultimeter:
    ...
    def set_fixed_range(self, value):
        ...
        원래 주석 처리돼 있던 코드는 CONF:VOLTage:DC:RANGe 를 썼는데 2100 에서
        유효한 문법이 아니다. SENSe:VOLTage:DC:RANGe 로 쓰면 auto range 는
        자동으로 꺼진다.
        """
        self.mutex.lock()
    ...
    def configure_burst(self, sample_count, multimeter_range=10, nplc=1):
        """
        버스트 판독 준비. 한 번의 READ? 로 sample_count 개 샘플을 받는다.

        기존 measure_voltage() 는 호출마다 MEASure? 를 보내 기기를 재설정하므로
        느리다. chopped 스캔은 CONFigure 1회 + READ? 반복 구조가 필요하다.
        """
        self.mutex.lock()
        self.keithmulti.write("*RST")
    ...
    def read_burst(self):
        """
        configure_burst 로 설정한 개수만큼 읽어 numpy array 로 반환한다.
