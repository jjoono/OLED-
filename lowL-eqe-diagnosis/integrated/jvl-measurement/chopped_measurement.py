...
class ChoppedSweep:
    ...
    def __init__(
        ...
        # 기록 시작 전에 같은 chop duty 로 소자를 돌리고 버리는 시간(초).
        # OLED 의 트랩 전하는 초 단위로 쌓이므로, 포인트 시작 직후에는 EL 이
        # 사이클마다 상승한다(chopped 준정상상태로 수렴하는 과도). 밝은
        # 포인트는 십수 사이클 만에 끝나서 평균 전체가 이 상승 구간에 놓이고,
    ...
    def _warmup(self, voltage):
        """기록 없이 같은 duty 로 chop 하면서 소자 과도를 보낸다."""
        if self.warmup_time <= 0:
            return
        warmup_start = time.time()
        while time.time() - warmup_start < self.warmup_time:
            self._measure_cycle(voltage)
            if self.stop:
                break
    ...
    def measure_point(self, voltage, quick=False):
        ...
        dark = False
        # 빛이 예상되는 포인트는 기록 전에 과도를 보낸다
        if not quick:
            self._warmup(voltage)
        ...
        while True:
            ...
            if quick and n >= self.quick_cycles:
                ...
                # quick 동안 잰 사이클들은 과도 상승 구간이므로 버리고,
                # 워밍업을 거친 뒤 처음부터 다시 기록한다
                quick = False
                self._warmup(voltage)
                deltas.clear()
                groups.clear()
        ...
        if dark or quick:
            ...
            # 전류(EQE 의 분모)는 DC 값이어야 한다. chopping 직후에는 트랩이
            # 덜 채워져 있어 전류도 DC 와 다르므로, 소자를 켠 채 워밍업한 뒤
            # 읽는다. 이러면 chopped 포인트에서도 분모는 연속 구간과 같은
            # DC 기준이 되고, 두 구간의 EQE 차이는 순수하게 광학(EL)만 남는다.
            time.sleep(max(self.warmup_time, self.settle_time))
