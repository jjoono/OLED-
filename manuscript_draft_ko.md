...
본 연구의 데이터는 이 경로를 예상치 못한 방향에서 뒷받침한다. lenslet class, 광원, 스택을 그대로 둔 채 텍스처 패치만 15 × 15 mm에서 25 × 25 mm로 넓히면 자연 배광이 측정 가능한 정도로 고각 쪽으로 이동한다. 0–20° 선택성이 Lambertian 값 0.117에서 0.096으로 떨어지는 반면, 40–60°와 60–80°는 0.337과 0.220에서 각각 0.360과 0.237로 올라간다. 메커니즘은 명확하다. 큰 횡방향 거리에서만 포집되는 빛은 기판 내에서 큰 전파각으로 진행한 성분이므로 비스듬히 출광한다. 즉 배광은 lenslet의 형상보다 **외부 광추출 필름의 크기**에 더 민감하며, 단일 기하 파라미터 하나가 전용 형상 최적화 전체에 필적하는 크기로 배광을 이동시킨다. 지향성이 중요한 응용에서는 필름 크기와 개구 구조 자체를 독립적인 최적화 변수로 다룰 필요가 있다.
