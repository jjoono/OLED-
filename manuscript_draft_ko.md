...
나머지 두 family는 미정이다: **[embedded MLA stress test 결과: 추후 삽입]**; **[randomly assembled MLA stress test 결과: 추후 삽입]**. 그림 5는 family당 3-panel 일반성 검사로 이 결과를 담을 예정이다. 세 signature가 재현되면 포화 진술은 단일 array 구성에서 시험한 네 개의 제조 가능 family로 확장되고, 어떤 family가 이탈한다면 그 이탈 자체가 어떤 구성 요소가 중요한지를 국소화한다.
