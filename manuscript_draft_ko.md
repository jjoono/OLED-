...
자유형(freeform) 마이크로렌즈 어레이(MLA)와 수치 역설계(inverse design)는 OLED의 광추출 효율과 배광을 동시에 제어할 수 있는 수단으로 널리 기대되어 왔다. 그러나 이 기대는 대개 점광원 또는 렌즈 개구보다 훨씬 작은 광원을 전제한 광학계에서 비롯되며, 균일한 확장 광원(extended source) 위에 공면적(coextensive)으로 타일링된 MLA에 그대로 적용되지는 않는다. 여기서 우리는 CPS 기반의 OLED 광원 모델과 3차원 광선추적을 결합하여, 반구형 기준 렌즈, 축대칭 자유형 렌즈, 비대칭·off-axis 자유형 렌즈를 동일한 기하·재료·공정 제약 아래 비교하였다. 총 외부양자효율(EQE)과 네 개의 polar band(0–20°, 20–40°, 40–60°, 60–80°) 각각을 독립적인 단일 목적함수로 최적화하였고, 총 EQE와 40–60° band 사이의 가중합 스윕으로 이를 보완하였다. 그 결과, 높은 형상 자유도와 cavity 공동 최적화에도 불구하고 자유형 MLA가 반구형 MLA 대비 제공하는 추가 이득은 탐색한 조건 전반에서 작았다. 동일 제약 아래 최적화한 hemispherical reference 대비, 최선의 freeform 설계는 어떤 polar objective에서도 7%를 넘게 앞서지 못했고 총 EQE는 1.3% 앞섰으며, 어느 목적에서도 반구를 밑돌지 않았다. 무작위 시드 탐색이 반구에 못 미쳤던 두 band는 탐색 인공물이었음을 각 탐색을 반구 최적해에서 재출발시켜 확인하였다. 결손은 회복되고 남는 이득은 +0.32%, +0.20%로 0과 구별되지 않는 반면, 동일한 절차가 실제로 여유가 있는 구간에서는 +4.70%를 되찾는다. 유효 무작위 설계 150개와 가중합 스윕($w=0,0.25,0.5,0.75,1$)을 합친 표본에서 총 EQE는 0.12–0.56에 걸쳐 4.5배 변했지만, 모든 (총 EQE, band EQE) 점은 준선형 경계로 붕괴하여 효율–지향성 트레이드오프는 관측되지 않았다. 총 EQE 상위 20개 설계에서 band 선택성은 사실상 불변이며(10–90% 폭이 각각 0.092–0.096, 0.277–0.281, 0.358–0.364, 0.233–0.238), 이 설계들이 서로 다른 네 개의 목적함수로 탐색되었음에도 그러하다. 특정 band를 전용 목적함수로 삼으면 배광은 실제로 움직인다. 선택성이 자연 분포 대비 최대 27%(0–20°) 및 20%(60–80°)까지 상승한다. 그러나 이는 공짜가 아니어서 같은 설계가 총 추출을 최대 11% 잃으며, 그 결과 전용 최적화가 '다른 목적을 추구하다 얻은 최선의 설계' 대비 확보하는 절대 band power 이득은 모든 구간에서 6%를 넘지 않았다. 이 benchmark family를 넘어, 우리는 두 가지 추가 제조 가능 외부 필름 MLA family—inverted(오목형) MLA와 pseudo-random supercell로 표현한 randomly assembled MLA—를 동일한 프로토콜로 stress test한다. Inverted family는 총 추출이 6% 낮은 상태에서 모든 signature를 재현하며, 최대 조향 예산도 60–80° band에서 1.19로 볼록 family가 다른 band에서 보인 1.20과 일치한다. 다만 동일한 patch·광원·스택 아래에서 자연 배광 자체가 조금 이동하는데(0.113/0.303/0.340/0.215 vs 0.094/0.278/0.361/0.236), 그 크기는 한 family 안에서 평균을 내는 방식을 바꿀 때 생기는 차이와 비슷한 수준이다. 무작위 조립 family는 lenslet 위치를 격자에서 흩뜨리고 각 lenslet에 독립적인 profile을 부여했음에도 같은 창(window) 안에 놓이며, 특정 무질서 실현에도 둔감하다(seed 3개에 대한 변동계수 0.2%). 세 가지 구성과 두 가지 평균 방식 전체에 걸쳐, 고효율 배광은 모두 0.09–0.11 / 0.28–0.30 / 0.34–0.36 / 0.21–0.24 안에 들어온다. 따라서 배광은 형상 자유도·array 구성·무질서 어느 것으로도 지휘할 수 있는 설계 변수가 아니라, **플랫폼(확장 광원·두꺼운 기판·공면적 외부 굴절 필름)의 성질**이다.
...
그래서 각 arm 의 탐색을 **그 arm 의 반구 최적해에서 재출발**시켜 이 모호함을 제거했다. 반구 점 자체를 시드에 넣고, 그 근방 섭동점들을 함께 넣고, 반구 점에서 국소 pattern search 를 직접 한 번 더 돌린다(4.3절). 반구 기준값은 이전 캠페인에서 읽어오지 않고 같은 세션에서 최종 정밀도로 다시 측정했으며, 두 값은 모든 arm 에서 0.1% 이내로 일치했다. 결과는 두 해석을 깨끗하게 갈라놓는다. 원래 캠페인이 못 미쳤던 두 band 에서는 결손이 회복된 뒤 거기서 멈춘다. 반구 대비 이득이 0–20°에서 +0.32%($t = 1.7$), 20–40°에서 +0.20%($t = 2.1$)로, 반복 3회 기준 문턱 2.13 아래다. 반면 실제로 여유가 있는 두 목적에서는 동일한 절차가 그것을 압도적으로 찾아낸다. 40–60°에서 +4.70%($t = 115$), 총 EQE 에서 +1.01%($t = 69$)이다. 4.7% 이득을 $t = 115$ 로 검출하는 절차가 같은 크기의 이득을 다른 곳에서 놓칠 리는 없다. 0 에 가까운 결과는 탐색이 약해서가 아니라 여유가 없어서 0 에 가까운 것이다(그림 2f).

따라서 1 미만의 gain 은 탐색 인공물이었고, 보정된 값은 1.003 과 1.002 다. 이 두 band 에서 형상 자유도는 반구를 회복시킬 뿐 그 너머로는 사실상 아무것도 사 오지 못한다. 일부 freeform 은 특정 angular bin 에서 국소적인 gain 을 보였지만, total EQE 감소, 다른 bin 으로의 power 이동, Monte-Carlo uncertainty 를 고려하면 hemispherical MLA 를 대체할 정도의 독립 성능 축을 형성하지 못했다.
...
전용 패치 크기 연구가 이 혼합 스케일을 직접 잰다(보충 표 S7). 고효율 설계 하나를 고정하고 텍스처 패치만 15 / 25 / 35 mm 로 바꿔 최종 정밀도로 재평가하면 총 EQE 는 0.5168 / 0.5428 / 0.5513 이다. 세 점은 감쇠길이 8.9 mm — 임계각 왕복 변위 2.32 mm 의 약 4배 — 인 포화 지수함수를 따르며, 무한 패치 극한은 0.5554 다. 즉 전 캠페인에 쓴 25 mm 패치는 무한 필름 값의 97.7% 를 담고, 외삽된 극한은 가중합 캠페인이 대형 패치 정규화에서 얻은 0.5556 을 0.04% 이내로 독립 재현한다 — 같은 양을 서로 다른 캠페인이 두 번 결정한 셈이다. 반면 배광은 이미 패치 수렴 상태다. 25 mm 와 35 mm 사이에서 어느 band 선택성도 0.11 %p 이상 변하지 않는다. 따라서 고정 패치의 절대 EQE 는 약 2.3% 잘린 하한이고, 동일 패치에서 평가한 이 논문의 모든 비율·상관·클래스 비교는 영향을 받지 않는다.
...
`calibrate_random_cost.mat`, `patch_convergence_result.mat`) 및 재활용 모델 출력(`angular_recycling_result.npz`,
