...
- 형상 설계로는 효율과 방향성을 동시에 향상시킬 수 없다
...
통과 기준으로 평면을 넘어설 수 없으며, 형상 설계로는 효율과 각도 선택성을 **교환할 수 있을 뿐 동시에
증가시킬 수 없다**. 각도 압축이 일반적으로 금지된 것은 아니며 그 대가는 **면적**이다. 다만 발광면을 덮는
...
대역 광량을 총 추출 효율과 각도 선택성의 곱 *P*<sub>band</sub> = η<sub>ext</sub> × *S*로 쓰면, 마스터
관계식은 이 **곱**을 유계한다.

$$\eta_{\mathrm{ext}}\times S \;\le\; \text{const}$$

형상 설계는 이 제약을 따라 구조를 이동시킬 수 있을 뿐 — 총 추출을 각도 집중과 맞바꾸거나 그 반대로 —
바깥으로 밀어낼 수 없다. 따라서 서로 독립적인 최적화로 얻어진 설계들은 개별 형상과 무관하게
(η<sub>ext</sub>, *S*) 평면에서 **공통의 등곱(iso-product) 곡선**을 이룰 것으로 예측된다.

> **[편집 주석 — 중요]** 이 예측은 **아직 검증 전**입니다. BO 결과를 (EQE_total, 선택성) 평면에 찍어
> 확인해야 합니다. **성립** → 논문 간판 결과 / **불성립** → "곱이 유계된다"만 남기고 삭제.
> **투고 전 반드시 검증할 것.**
...
### 4.2 효율–선택성 트레이드오프
[Fig 2b] 설계들은 (η<sub>ext</sub>, *S*) 평면에서 공통 곡선을 이루며 2.3절의 유계된 곱과 부합한다.
**[검증 후 확정]**
...
노력은 2.5절이 지목한 레버들 — 각도 선별을 통한 휘도 축적, 마이크로캐비티를 통한 광원 배광 조절, 그리고
...
이미 이 상한을 달성하므로, 역설계가 반구형 프로파일에서 포화하는 현상은 최적화가 아니라 보존의 귀결이며,
형상 설계로는 효율과 방향성을 교환할 수 있을 뿐이다. 각도 압축은 출광면/광원 면적비가 1을 넘을 것을
...
| 2 | 단일 통과 상한, 평면 포화, iso-product 평면, frontier | **iso-product 검증 필요** |
