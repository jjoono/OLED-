% Made by JH Kim (SL version, thick transport layers)
% Same as planar_Sweep_SL_JH_full_lambda.m but with thicker, FIXED ETL/HTL:
%   ETL (B3PyMPM)      : 50 nm -> 80 nm
%   HTL (TAPC, each)   : 47 nm -> 60 nm
% Purpose: show with a contour plot over (S, g) that the final EQE with an
% index-matched external scattering layer is INSENSITIVE to the exact choice
% of S and g (no need to hit an optimum).
...
load('BSDF_SL_S_1to15by2_G_0to1by0.1_nSL_151.mat')   % BSDF_SL, S_list, g_list, n_SL
...
thickness=[100 80 25 10 60 3 60 3 60 3 150];   % thick ETL (80) / thick HTL (60 each)
EML_position=4;
...
save('SL_sweep_thick_transport_result.mat','data_matrix','Power_matrix','EQE_SL_map','Enhancement_map','I_SL_angle', ...
     'S_list','g_list','n_SL','EQE_air','EQE_sub','EQE_wg','EQE_spp','EQE_abs','R_step','thickness','-v7');

%% contour plot (EQE vs S and g)
% g = 1 is the degenerate no-scattering limit (flat interface); exclude it
% from the contour so the plateau of the real scattering regime is visible.
g_plot=g_list(g_list<1);
EQE_plot=EQE_SL_map(:,g_list<1);

figure;
contourf(g_plot,S_list,EQE_plot,20,'LineColor','none');
hold on;
contour(g_plot,S_list,EQE_plot,0.40:0.02:0.60,'k','ShowText','on');
xlabel('g (HG asymmetry parameter)');
ylabel('S (scattering optical thickness)');
title(sprintf('EQE with scattering layer (planar EQE = %.3f)',EQE_air));
colorbar;
caxis([0 max(EQE_plot(:))]);

fprintf('\n--- robustness over the (S,g) grid (g < 1) ---\n');
fprintf('EQE min  = %.4f, EQE max = %.4f\n',min(EQE_plot(:)),max(EQE_plot(:)));
fprintf('spread   = %.1f %% of the mean\n',100*(max(EQE_plot(:))-min(EQE_plot(:)))/mean(EQE_plot(:)));
fprintf('planar   = %.4f (every grid point is a x%.2f-x%.2f enhancement)\n', ...
    EQE_air,min(EQE_plot(:))/EQE_air,max(EQE_plot(:))/EQE_air);

print('SL_EQE_contour_thick_transport.png','-dpng','-r150');
