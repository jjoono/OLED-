...
%
%  [실행 결과 2026-08-14] arm 1,2,3,5 실행 (7.6 h, arm 당 108-118 분):
%      0-20   +0.32%  t=1.7    -> 정체
%      20-40  +0.20%  t=2.1    -> 정체 (문턱 2.13 바로 아래)
%      40-60  +4.70%  t=114.6  -> 개선
%      total  +1.01%  t=68.7   -> 개선
%    즉 시험이 판별에 성공했다. 실제 여유가 있는 곳에서는 t>68 로 검출하면서
%    문제의 두 band 에서는 0 에 붙었으므로, 그 두 값은 탐색 실패가 아니라
%    여유의 부재다. 원래 캠페인의 G_j = 0.946 / 0.966 은 탐색 인공물이었고
%    보정값은 1.003 / 1.002 다. 반구 재측정은 아카이브와 0.1% 이내 일치.
%    arm 4 는 원래 G_4 = 1.020 으로 이미 1을 넘어 미실행.
%
...
ARMS_TO_RUN = [4];                % [] = 전부. 1,2,3,5 는 2026-08-14 완료 -> merge 로 보존됨
