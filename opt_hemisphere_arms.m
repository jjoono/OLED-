% ============================================================
%  opt_hemisphere_arms.m
%
%  목적: 원고 2.2절의 relative gain
%          G_j = max[EQE_j | freeform] / max[EQE_j | hemisphere]
%        의 분모를 채운다. 반구형 MLA를 freeform 과 **동일한 제약·동일한
%        스택·동일한 patch·동일한 fidelity** 로 각 목적함수마다 최적화한다.
%
%  [무엇을 고정하고 무엇을 여는가]
%    형상 5개 제어점을 사분원(반구) 위에 고정한다. 즉 x2..x6 / y2..y6 은
%    (sin15,cos15) ... (sin75,cos75) 로 고정. 자유변수는 cavity 와 높이,
%    즉 dETL / dHTL / stretchZ 3개뿐이다.
%    -> 2.1절이 약속한 "hemispherical MLA 도 동일한 outer radius 및 공정 가능한
%       높이 범위에서 최적화" 를 그대로 구현한다. 반구에 불리한 조건을 주면
%       G_j 가 부풀려지므로, 반구에도 cavity/높이 최적화를 똑같이 허용한다.
%
%  [arm 구성] freeform 쪽(opt_4band_freeform.m)과 같은 5개:
%        0-20, 20-40, 40-60, 60-80 도 band + EQE_total
%
%  [예산] 자유변수가 3개뿐이라 freeform(13변수)보다 적은 예산으로 수렴한다.
%        arm 당 surrogateopt 30 + patternsearch 10, 승자 고정밀 3회 재평가.
%
%  [출력] opt_hemisphere_result.mat
%         + opt_4band_result.mat 이 있으면 G_j 표를 바로 계산해 출력
%
%  기반: opt_4band_freeform.m (objFcn_both / isValidPoints / RenewLightTools 등
%        검증된 함수를 그대로 재사용, 수정 없음)
% ============================================================
clear;
...
PATCH_XY = 25;              % 전 캠페인 통일값
GEOM_TOL = 1e-4;
GEOM_MISMATCH_LOG = [];
REQUIRE_MONOTONIC_X = true; % 반구 제어점은 이미 단조이므로 무해
...
%% ===== fidelity (freeform arm 과 반드시 동일) =====
WAVE_N_SEARCH = 10;   WAVE_N_FINAL = 2;
RAY_SEARCH    = 10000; RAY_FINAL   = 50000;
N_FINAL_REP   = 3;

%% ===== 반구 형상 (고정) =====
%  사분원 위 5점: theta = 15,30,45,60,75 도 -> (sin, cos)
th = [15 30 45 60 75];
HEMI_X = sind(th);          % [0.2588 0.5000 0.7071 0.8660 0.9659]
HEMI_Y = cosd(th);          % [0.9659 0.8660 0.7071 0.5000 0.2588]
if ~isValidPoints([HEMI_X HEMI_Y 60 60 1])
    error('반구 제어점이 기하 제약을 통과하지 못한다. th 배치를 확인할 것.');
end

%% ===== 자유변수: dETL, dHTL, stretchZ =====
varNames3 = {'dETL','dHTL','stretchZ'};
lb3 = [10, 10, 0.1];
ub3 = [150,150, 3];
expand = @(v) [HEMI_X, HEMI_Y, v(1), v(2), v(3)];   % 3 -> 13 차원

%% ===== arm 구성 (freeform 과 동일) =====
BAND_LIST  = {[0 20], [20 40], [40 60], [60 80], [0 90]};
BAND_NAMES = {'0-20 deg','20-40 deg','40-60 deg','60-80 deg','EQE_total'};
BAND_COL   = [1, 2, 3, 4, 0];      % 0 = 총 EQE
nArm = numel(BAND_LIST);

EVALS_PER_ARM = 30;    % 3변수라 freeform 의 60 보다 적어도 충분
POLISH_EVALS  = 10;
N_SEED        = 8;

EVAL_LOG = [];
EVAL_PHASE = 8;        % 8 = hemisphere arm (freeform 4, inverted 6, random 7)
...
hemi_x    = nan(nArm, 3);
hemi_val  = nan(nArm, 1);    % arm 목적값 (고정밀)
hemi_tot  = nan(nArm, 1);
hemi_bins = nan(nArm, 4);

RESULT_MAT = 'opt_hemisphere_result.mat';

%% =====================================================================
for k = 1:nArm
    bcol = BAND_COL(k);
    fprintf('\n########## HEMISPHERE ARM %s (%d/%d) ##########\n', ...
            BAND_NAMES{k}, k, nArm);
    RenewLightTools();
    lt = ltloc.GetLTAPI(ID_swept);
    ltml.LTSetOption(lt, "ShowFileDialogBox", 0);
    eval_count = 0;
    EVAL_W = tern0(bcol==0, -1, BAND_LIST{k}(1));
    ray_nums_current = RAY_SEARCH;  wave_n_current = WAVE_N_SEARCH;

    seed3 = lb3 + rand(N_SEED,3) .* (ub3 - lb3);
    sopts = optimoptions('surrogateopt', ...
        'MaxFunctionEvaluations', EVALS_PER_ARM, ...
        'MinSurrogatePoints', 12, ...
        'InitialPoints', struct('X', seed3), ...
        'UseParallel', false, 'PlotFcn', [], 'Display','iter');

    objf = @(v) hemi_obj(expand(v), bcol);
    [vS, ~] = surrogateopt(objf, lb3, ub3, sopts);
    if isempty(vS), fprintf('  [Warn] feasible 해 미반환\n'); continue; end
    v0 = vS(:).';
    try
        vP = patternsearch(@(v) hemi_scalar(expand(v), bcol), v0, ...
                           [],[],[],[], lb3, ub3, [], psOpts);
        vP = vP(:).';
    catch
        vP = v0;
    ...
    % 저정밀 기준으로 더 좋은 후보 채택
    cands = {v0};  if ~isequal(vP, v0), cands{end+1} = vP; end
    best = -inf; vBest = [];
    for c = 1:numel(cands)
        [et, bins] = simulate_bands(expand(cands{c}));
        if ~isfinite(et), continue; end
        sc = tern0(bcol==0, et, bins(max(bcol,1)));
        if sc > best, best = sc; vBest = cands{c}; end
    end
    if isempty(vBest), continue; end

    % 고정밀 재평가 (freeform arm 과 동일 설정)
    ray_nums_current = RAY_FINAL;  wave_n_current = WAVE_N_FINAL;
    er = nan(N_FINAL_REP,1);  br = nan(N_FINAL_REP,4);
    for r = 1:N_FINAL_REP
        [er(r), br(r,:)] = simulate_bands(expand(vBest));
    end
    hemi_x(k,:)    = vBest;
    hemi_tot(k)    = mean(er,'omitnan');
    hemi_bins(k,:) = mean(br,1,'omitnan');
    hemi_val(k)    = tern0(bcol==0, hemi_tot(k), hemi_bins(k, max(bcol,1)));
    fprintf('  >>> %s : 목적값=%.5f  EQE_total=%.5f  (dETL=%.1f dHTL=%.1f sZ=%.2f)\n', ...
        BAND_NAMES{k}, hemi_val(k), hemi_tot(k), vBest(1), vBest(2), vBest(3));

    save(RESULT_MAT,'EVAL_LOG','hemi_x','hemi_val','hemi_tot','hemi_bins', ...
         'BAND_NAMES','BAND_COL','HEMI_X','HEMI_Y','varNames3','lb3','ub3', ...
         'PATCH_XY','GEOM_MISMATCH_LOG');
end

%% =====================================================================
%  G_j 표 (freeform 결과가 있으면 즉시 계산)
%% =====================================================================
fprintf('\n################ HEMISPHERE SUMMARY ################\n');
fprintf('%-12s | %10s | %10s\n','arm','목적값','EQE_total');
for k = 1:nArm
    fprintf('%-12s | %10.5f | %10.5f\n', BAND_NAMES{k}, hemi_val(k), hemi_tot(k));
end

FF = 'opt_4band_result.mat';
if exist(FF,'file')
    D = load(FF);
    if isfield(D,'band_eqe_hi')
        fprintf('\n---- relative gain  G_j = freeform / hemisphere ----\n');
        fprintf('%-12s | %10s | %10s | %8s\n','arm','freeform','hemisphere','G_j');
        Gj = nan(nArm,1);
        for k = 1:nArm
            if k <= numel(D.band_eqe_hi) && isfinite(D.band_eqe_hi(k)) && isfinite(hemi_val(k))
                Gj(k) = D.band_eqe_hi(k) / hemi_val(k);
                fprintf('%-12s | %10.5f | %10.5f | %8.3f\n', ...
                        BAND_NAMES{k}, D.band_eqe_hi(k), hemi_val(k), Gj(k));
            end
        end
        g = Gj(isfinite(Gj));
        if ~isempty(g)
            fprintf('\n  G_j 범위: %.3f - %.3f  (원고 2.2절에 이 범위를 넣는다)\n', ...
                    min(g), max(g));
        end
        save(RESULT_MAT,'Gj','-append');
    end
else
    fprintf('\n[참고] %s 이 없어 G_j 는 계산하지 않았다. 나중에 이 스크립트를\n', FF);
    fprintf('       다시 실행하거나 두 .mat 을 함께 불러 계산할 것.\n');
end
report_geom_rejection(GEOM_MISMATCH_LOG, GEOM_TOL);
fprintf('\nsaved -> %s\n########## 완료 ##########\n', RESULT_MAT);


%% ===== 반구 arm 목적함수 (surrogateopt: 제약 결합형) =====
function out = hemi_obj(x13, bcol)
[et, bins] = simulate_bands(x13);
val = tern0(bcol==0, et, bins(max(bcol,1)));
if ~isfinite(et) || ~isfinite(val)
    out.Ineq = 1;  out.Fval = 1;
else
    out.Ineq = -1; out.Fval = -val;
end
end

%% ===== 반구 arm 목적함수 (patternsearch) =====
function f = hemi_scalar(x13, bcol)
[et, bins] = simulate_bands(x13);
val = tern0(bcol==0, et, bins(max(bcol,1)));
