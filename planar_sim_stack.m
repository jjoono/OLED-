function out = planar_sim_stack(no_bar, ne_bar, thickness_in, EML_position, emission_in, eta_rad, hdr, z0_in)
% Generic planar air-mode dipole simulation for an arbitrary top-emission stack.
%   no_bar/ne_bar : 401 x N ordinary/extraordinary index matrices (400:800 nm),
%                   column 1 = medium behind the reflector, column N = outcoupling medium (air)
%   thickness_in  : 1 x (N-2) layer thicknesses (nm) for columns 2..N-1
%   EML_position  : column index of the emitting layer
%   emission_in   : 401 x 1 emission spectrum (photon-number weighted, arbitrary scale)
%   z0_in         : dipole distance (nm) from the reflector-side interface of the EML
% Same physics as planar_sim_airmode.m (fixed TMF_birefringence_whole*).
wavelength=(400:800)';
wavelength_num=length(wavelength);
sin089=sind(0:89);
emission_spectrum=emission_in(:);
